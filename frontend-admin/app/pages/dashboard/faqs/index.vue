...
const { items, loading, saving, error, load, create, update, remove } = useCrud<{ id: number; question: string; answer: string }>('/admin/faqs', 'FAQ')
...
<template>
  <div>
    ...
    <AppTable :cols="[{ key: 'question', label: 'Question' }, { key: 'answer', label: 'Answer' }]" :rows="items" :loading="loading">
      <template #answer="{ row }">{{ (row as any).answer?.slice(0, 80) }}â€¦</template>
