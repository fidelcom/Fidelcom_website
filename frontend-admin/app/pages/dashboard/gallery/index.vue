...
const { items, loading, saving, error, load, create, update, remove } = useCrud<{ id: number; name: string; image: string; alt_text: string }>('/api/v1/admin/gallery')
  ...
  <div>
    ...
    <div v-else class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
      <div v-for="img in items" :key="img.id" class="group relative rounded-xl overflow-hidden bg-surface aspect-square">
        <img :src="img.image" :alt="img.alt_text || img.name" class="w-full h-full object-cover" />
